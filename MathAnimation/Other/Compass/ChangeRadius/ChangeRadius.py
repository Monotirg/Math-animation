...
class ChangeRadiusV1(Scene):
    def construct(self):
        ...
        center_new_coords = 0.5*(dot_left.get_center() + dot_right_new.get_center()) + h*np.array([0,1,0])
        ...
        vec_sum1 = Line(dot_left.get_center(), dot_right_new.get_center(), buff=0.15, color=BLACK, stroke_width=6).set_z_index(-1000)
        vec_sum2 = Line(
        ...
        line_left = Line(dot_left.get_center(), dot_center_new.get_center(), color=BLACK, buff=0.15, stroke_width=6).set_z_index(-1000)
        ...
        height_formula = MathTex(r"|\vec{h}| = \sqrt{l^2 - \frac{r^2}{4}}", color=BLACK).to_edge(LEFT).shift(RIGHT)
        
        self.add(dot_left, dot_right, dot_center, line_left_start)
        self.add(label_left, label_right, label_center, line_right_start)
        self.wait(0.4)
        self.play(FadeOut(label_left, label_right, label_center, line_left_start, line_right_start), run_time=0.8)
        ...
        self.play(Create(line_left),FadeIn(label_l,label_height, label_r, dot_center_new), run_time=0.8)
        self.play(AnimationGroup(
            ...
            FadeIn(height_formula[0][0], height_formula[0][3:7], height_formula[0][8:10], height_formula[0][11:]),
            lag_ratio = 0.4
        ))
        self.wait(0.2)
    
        line_right_start = Line(dot_center_new.get_center(), dot_left.get_center(), color=BLACK, stroke_width=6, buff=0.15)
        line_left_start = Line(dot_center_new.get_center(), dot_right.get_center(), color=BLACK, stroke_width=6, buff=0.15)

        self.play(AnimationGroup(
            FadeOut(vec_sum1, vec_sum3, dot_mid, height_formula, label_r, label_height, label_l, line_left),
            FadeIn(label_left, label_right, label_center, line_left_start, line_right_start), 
            lag_ratio = 0.3
        ))
        self.wait(0.4)