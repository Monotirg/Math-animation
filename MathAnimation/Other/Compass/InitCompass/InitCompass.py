...
class InitCompass(Scene):
    def construct(self):
        ...
        vec_left = (dot_left.get_center() - dot_center.get_center())/np.linalg.norm(dot_left.get_center() - dot_center.get_center())
        vec_right = (dot_right.get_center() - dot_center.get_center())/np.linalg.norm(dot_right.get_center() - dot_center.get_center())
        per_left = per_line(vec_left)
        
        label_length_left = Text("length", color=BLACK).rotate(-np.arctan2(vec_left[1], vec_right[0]))
        label_length_left.next_to(vec_left, UP).shift(-per_left)
        label_length_right = Text("length", color = BLACK).rotate(np.arctan2(vec_left[1], vec_right[0]))
        label_length_right.move_to([-label_length_left.get_center()[0], label_length_left.get_center()[1], 0])
        ...
        self.play(AnimationGroup(
            compass1.animate.shift(RIGHT*(compass2.get_center()[0] - compass1.get_center()[0])),
            compass3.animate.shift(RIGHT*(compass2.get_center()[0] - compass3.get_center()[0]))
        ))
