...
class ChangeAngle(Scene):
    def construct(self):
        ...
        vec_vertical = Line(
            dot_center.get_center(),
            0.5 * (dot_left.get_center() + dot_right.get_center()),
            color=BLACK,
            stroke_width=6,
            buff=0.15,
        )
        ...
        vec_left_label = (
            MathTex(r"\vec{v}_1", color=BLACK)
            .next_to(vec_left, LEFT)
            .shift(0.5 * RIGHT)
        )
        vec_right_label = (
            MathTex(r"\vec{v}_2", color=BLACK)
            .next_to(vec_right, RIGHT)
            .shift(0.5 * LEFT)
        )
        ...
        formula = MathTex(
            r"\theta = \arccos{\frac{(\vec{v}_1,\vec{v}_2)}{|\vec{v}_1||\vec{v}_2|}}",
            color=BLACK,
        ).to_edge(LEFT)
        ...
        self.wait(0.2)
        self.play(
            FadeOut(vec_left_label, vec_right_label, formula, angle, angle_label),
            run_time=0.8,
        )
        self.play(GrowFromPoint(vec_vertical, vec_vertical.get_start()))

        angle = always_redraw(
            lambda: Angle(vec_vertical, vec_right_copy, radius=0.9, color=BLACK)
        )
        angle_label = (
            MathTex(r"\frac{\theta}{2}", color=BLACK)
            .next_to(angle, DOWN)
            .shift(RIGHT * 0.05)
        )
        angle_label_new1 = (
            MathTex(r"\frac{\alpha}{2}", color=BLACK)
            .next_to(angle, DOWN)
            .shift(0.4 * RIGHT + 0.1 * UP)
        )
        ...
        angle_label_new2 = (
            MathTex(r"\gamma", color=BLACK)
            .move_to(angle_label_new1.get_center())
            .shift(0.2 * UP + 0.2 * RIGHT)
        )
        ...
        formula = MathTex(
            r"\gamma = \frac{1}{2}(\theta - \alpha)", color=BLACK
        ).to_edge(RIGHT)
        formula.shift(
            [0, angle_label_new2.get_bottom()[1] - formula[0][0].get_bottom()[1], 0]
        )
