...
class RotateCompass(ZoomedScene):
    def construct(self):
        ...
        line_left = Line(dot_center.get_center(), dot_left.get_center(), buff=0.15, color=BLACK)
        line_right = Line(dot_center.get_center(), dot_right.get_center(), buff=0.15, color=BLACK)
        ...
        vec_shift = Line(dot_center.get_center(), np.array([-2, -1, 0]), color=BLACK, stroke_width=4)
        vec_shift.add_tip(tip_length=0.25, tip_width=0.25)
        vec_shift_reverse = Line(np.array([-2, -1, 0]),dot_center.get_center(), color=BLACK, stroke_width=4)
        vec_shift_reverse.add_tip(tip_length=0.25, tip_width=0.25)
        
        rot_mat = Matrix([[r"\cos{\alpha}", r"-\sin{\alpha}"], [r"\sin{\alpha}", r"\cos{\alpha}"]], 
            left_bracket="(",
            right_bracket=")", 
            bracket_h_buff=0.1, 
            h_buff=2
        ...
        self.play(AnimationGroup(
            VGroup(compass, lines).animate.shift(-compass_labels[0][2].get_center() + np.array([-2, -1, 0])),
            FadeOut(vec_shift)
        ), run_time=1.2)
        self.play(FadeIn(rot_mat), run_time=0.8)
        self.play(AnimationGroup(
            ReplacementTransform(rot_mat, dot_center),
            Rotate(VGroup(compass, lines), angle=2*PI/7, about_point=dot_center.get_center()),
            lag_ratio=0.5
        ), run_time=1.2)
        self.play(DrawBorderThenFill(vec_shift_reverse), run_time=1.2)
        self.play(AnimationGroup(
            VGroup(compass, lines).animate.shift(-compass_labels[0][2].get_center() + vec_shift_reverse.get_end()),
        ...
        self.play(self.camera.frame.animate.scale(0.5).move_to(compass.get_center()), run_time=1.2)
