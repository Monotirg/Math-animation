...
def create_ruler(
    ...
    width_division_ident: float = 0.05,
    height_division: float = 0.45,
    height_subdivision: float = 0.25,
) -> VGroup:
    shape = np.array([[0, 0, 0], [width, 0, 0], [width, -height, 0], [0, -height, 0]])
    ruler_shape = Polygon(*shape, color=BLACK)

    step_division = (1 - width_division_ident) * width / (count_division - 1)
    div_x = np.arange(0.5 * width_division_ident * width, width, step_division)
    ...
    if count_subdivision > 0:
        ...
        temp += 0.5 * width_division_ident * width
        subdiv_x = np.array(
            [temp + step_division * i for i in range(count_division - 1)]
        )
