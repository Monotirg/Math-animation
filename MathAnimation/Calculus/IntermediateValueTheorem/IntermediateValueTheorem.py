...
class GraphFunc(Scene):
    def construct(self):
        ...
        func  = lambda x: (x**3)*11/10 - (x**2)*58/5 + x*301/10 - 73/5 + 25
        ...
        line_min = axes.get_lines_to_point(axes.c2p(x_min,func(x_min))).set_z_index(dot_min.get_z_index()-1)
        line_max = axes.get_lines_to_point(axes.c2p(x_max,func(x_max))).set_z_index(dot_max.get_z_index()-1)
        label_b = MathTex("a", font_size=40).move_to(line_min[1].get_bottom()).shift(0.3*DOWN)
        label_a = MathTex("b", font_size=40).move_to(line_max[1].get_bottom()).align_to(label_b, DOWN)
        label_B = MathTex("A", font_size=40).move_to(line_min[0].get_left()).shift(0.3*LEFT)
        label_A = MathTex("B", font_size=40).move_to(line_max[0].get_left()).align_to(label_B,LEFT)
        ...
        label_C.add_updater(lambda item: item.move_to(horizontal_line.get_left()).shift(0.3*LEFT))        
        
        self.play(DrawBorderThenFill(axes), run_time =0.5, rate_func=rate_functions.ease_in_out_quad)
        ...
        self.play(Create(line_min[0].rotate(PI)), Create(line_max[0].rotate(PI)), run_time =0.5)
        self.play(FadeIn(label_A, label_B), run_time=0.75)
        self.play(Create(vertical_line), Create(horizontal_line), run_time=0.5)
        self.play(FadeIn(dot, label_c, label_C), FadeOut(label_a, label_b, label_A, label_B), run_time=0.75)
