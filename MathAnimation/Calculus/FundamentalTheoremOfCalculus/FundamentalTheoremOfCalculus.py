...
class FundamentalTheoremOfCalculus(Scene):
    def construct(self):
        ...
        self.play(
            DrawBorderThenFill(rect[0], rate_func=smooth), lag_ratio=0.5, run_time=0.6
        )
        self.wait(1)
        self.play(
        ...
        r = axes.get_area(
            graph, x_range=[1, 8], color="#005EFF", opacity=0.9, stroke_width=0
        )
        r.set_z_index(1)
        self.add(r)
