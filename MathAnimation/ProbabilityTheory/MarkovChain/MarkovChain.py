...
class MarkovChain(Scene):
    def construct(self):
        vertex1 = Circle(
            radius=0.5, color="#E5E5E5", fill_color="#E5E5E5", fill_opacity=1
        )
        ...
        arrow_color = YELLOW
        arrow12 = Arrow(
            vertex1.get_center(), vertex2.get_center(), color=arrow_color, buff=0.5
        ).shift(0.15 * UP)
        arrow13 = Arrow(
            vertex1.get_center(), vertex3.get_center(), color=arrow_color, buff=0.5
        ).shift(0.15 * RIGHT)
        arrow21 = Arrow(
            vertex2.get_center(), vertex1.get_center(), color=arrow_color, buff=0.5
        ).shift(0.15 * DOWN)
        arrow23 = Arrow(
            vertex2.get_center(), vertex3.get_center(), color=arrow_color, buff=0.5
        )
        arrow31 = Arrow(
            vertex3.get_center(), vertex1.get_center(), color=arrow_color, buff=0.5
        ).shift(0.15 * LEFT)
        arrow34 = Arrow(
            vertex3.get_center(), vertex4.get_center(), color=arrow_color, buff=0.5
        )
        arrow42 = Arrow(
            vertex4.get_center(), vertex2.get_center(), color=arrow_color, buff=0.5
        )
        ...
        all_arrow = VGroup(
            arrow12, arrow21, arrow13, arrow23, arrow31, arrow34, arrow42
        )
        all_label = VGroup(
            label12, label13, label21, label23, label31, label34, label42
        )

        target_vertex = Circle(
            radius=0.5, color=YELLOW, fill_color=YELLOW, fill_opacity=1
