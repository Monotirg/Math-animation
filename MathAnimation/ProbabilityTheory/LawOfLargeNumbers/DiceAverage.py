...
class DiceAverage(Scene):
    def construct(self):
        ...
        dice = VGroup(*[VGroup(*[
            create_dice(val, 0.75, 0.2) for val in dice_value[i]
        ]).arrange(RIGHT, buff=0.2) for i in range(dice_value.shape[0])])
        cumsum_mean  = np.array([val/(i+1) for i, val in enumerate(np.cumsum(dice_value))])
        ...
        graph = axes.plot_line_graph(
            np.arange(1, dice_value.shape[1] + 1), cumsum_mean[0], 
            add_vertex_dots=False, line_color="#FDE910"
        )
        ...
        for i in range(1, dice_value.shape[0]):
            ...
            self.play(AnimationGroup(
                AnimationGroup(*[
                    GrowFromPoint(d, [d.get_bottom()[0], -5, 0]) for d in dice[i]
                ], lag_ratio=0.2),
                Create(graph),
