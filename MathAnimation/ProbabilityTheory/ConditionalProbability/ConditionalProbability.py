...
class ConditionalProbability(Scene):
    def construct(self):
        circle_left = Circle(
            radius=2, color=BLUE, stroke_width=1, fill_color=BLUE, fill_opacity=0.7
        ).shift(RIGHT)
        circle_right = (
            Circle(
                radius=2, color=RED, stroke_width=1, fill_color=RED, fill_opacity=0.8
            )
            .set_z_index(circle_left.z_index - 1)
            .shift(LEFT)
        )
        formula = MathTex(r"P(A|B) = \frac{P(A \cap B)}{P(B)}")
        formula.set_z_index(circle_left.z_index + 1)
        ...
        intersection_cirlce = Intersection(
            circle_left, circle_right, color=GREEN, fill_color=GREEN, fill_opacity=0.5
        ).set_z_index(circle_left.z_index + 1)
